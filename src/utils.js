...
export const getTextEntity = (text) => {
  ...
  textEl.style.fontFamily = "TF Build";
  textEl.setAttribute("position", "0 2.8 -10");
  return textEl;
};
...
export const getAgeEntity = (age) => {
  ...
  ageDigits.forEach((digit, i) => {
    if (Number.isNaN(parseInt(digit, 10)) === false) {
      ...
      digitEl.setAttribute(
        "gltf-model",
        `url(./assets/models/digits/${digit}.glb)`
      );
  ...
  ageEl.setAttribute(
    "position",
    `-${(ageEl.children.length - 1) * 0.05} 1.55 -3.69`
  );
  return ageEl;
};
